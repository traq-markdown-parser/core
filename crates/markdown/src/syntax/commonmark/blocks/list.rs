...
pub(super) fn parse(
    ...
    while i < lines.len() {
        ...
        children.push(DraftNode::blocks(
            source.span(lines[item_start].start, lines[i - 1].end),
            NodeKind::ListItem {
                marker: marker.text,
            },
            source.select(&ranges),
        ));
    }
    let mut node = DraftNode::nodes(
        source.span(lines[first].start, lines[i - 1].end),
        NodeKind::List {
    ...
    let loose = node.children().iter().any(|item| {
        item.is_loose()
            || item
                .children()
                .iter()
                .filter(|child| matches!(child.kind, NodeKind::Paragraph))
                .count()
                > 1
    });
    if let NodeKind::List { tight, .. } = &mut node.kind {
        *tight &= !loose;
    }
}
