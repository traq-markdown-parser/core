...
型付きルール、text provider、SourceView、Budget の説明は [parser core](../crates/core/parser/README.md)、
組み合わせは [文法 preset](../crates/trap/traq/README.md) を参照してください。
...
```ts
import { isKnownNode } from '@traptitech/markdown-parser'
...
こちらは同じ native Document を借用でき、AST の JSON 変換を挟みません。

保存済みメッセージの文法版と Parser の対応は利用側が管理します。SDK / AST に syntaxVersion はありません。
原文を適切な preset で再 parse する運用で、永続 AST の互換層は設けません。
LRU / TTL の自動キャッシュは導入していません。
